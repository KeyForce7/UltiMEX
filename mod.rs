...
pub fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
 ...
 //taunt 
if StatusModule::situation_kind(module_accessor) == SITUATION_KIND_AIR && status_kind != *FIGHTER_STATUS_KIND_FALL {
    ...
    if ControlModule::check_button_trigger(module_accessor, *CONTROL_PAD_BUTTON_APPEAL_HI){
      ...
      if AIRTAUNT_USED[ENTRY_ID] == false{
        WorkModule::on_flag(module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLAG_SUPERLEAF);
        WorkModule::set_int(module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_SUPERLEAF_FALL_SLOWLY_FRAME, MotionModule::end_frame(module_accessor) as i32);
        
      } else if AIRTAUNT_USED[ENTRY_ID] ==  true{
        WorkModule::off_flag(module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLAG_SUPERLEAF);
      }
      CancelModule::enable_cancel(module_accessor);
...
//shield drop no lag, disable roll out of shield
...
if status_kind == *FIGHTER_STATUS_KIND_DASH && ControlModule::get_stick_y(module_accessor)<=0.3 {
  MotionModule::change_motion_force_inherit_frame(module_accessor, smash::phx::Hash40::new_raw(MotionModule::motion_kind(module_accessor)), 0.0, 1.0, 0.0);
}

//up b jump cancel wip might delete

/*if status_kind ==*FIGHTER_STATUS_KIND_SPECIAL_HI{
  if WorkModule::get_int(module_accessor, *FIGHTER_INST)
  WorkModule::enable_transition_term_group_ex(module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_AERIAL);
  WorkModule::enable_transition_term_group_ex(module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_JUMP_SQUAT)
}*/
//floating
