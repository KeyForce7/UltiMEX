...
static mut AIRDODGE : [i32; 8] = [2; 8];
static mut POSX : [f32; 8] = [-1.0 ; 8];
static mut OPPONENT_ID : [usize; 8] = [9; 8];
static mut AIRTAUNT_USED :[bool; 8] =[false;8];
#[skyline::hook(replace = smash::app::lua_bind::CancelModule::is_enable_cancel)]
pub unsafe fn is_enable_cancel_hook(module_accessor: &mut smash::app::BattleObjectModuleAccessor) -> bool {
  let ret = original!()(module_accessor);

  if StatusModule::status_kind(module_accessor) == StatusModule::prev_status_kind(module_accessor, 0) && StatusModule::status_kind(module_accessor) == *FIGHTER_STATUS_KIND_ATTACK_S3 /*disable for now || (StatusModule::status_kind(module_accessor) == StatusModule::prev_status_kind(module_accessor, 1) && [*FIGHTER_STATUS_KIND_ATTACK_HI4_HOLD, *FIGHTER_STATUS_KIND_ATTACK_S4_HOLD, *FIGHTER_STATUS_KIND_ATTACK_LW4_HOLD, *FIGHTER_STATUS_KIND_ATTACK_HI4_START, *FIGHTER_STATUS_KIND_ATTACK_S4_START,*FIGHTER_STATUS_KIND_ATTACK_LW4_START].contains(&StatusModule::status_kind(module_accessor)))*/ {
...
pub fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
  ...
  //side special out of a run
    ...
    //up b fix
    if (status_kind == *FIGHTER_STATUS_KIND_SPECIAL_HI || MotionModule::motion_kind(module_accessor) == smash::hash40("special_hi_air") || MotionModule::motion_kind(module_accessor) == smash::hash40("special_hi")){
      if ControlModule::check_button_trigger(module_accessor, *CONTROL_PAD_BUTTON_JUMP){
        if StatusModule::situation_kind(module_accessor) == SITUATION_KIND_AIR && WorkModule::get_int(module_accessor,*FIGHTER_INSTANCE_WORK_ID_INT_JUMP_COUNT)< WorkModule::get_int(module_accessor,*FIGHTER_INSTANCE_WORK_ID_INT_JUMP_COUNT_MAX){
